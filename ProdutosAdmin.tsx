...
export default function ProdutosAdmin() {
  ...
  const produtosFiltrados=useMemo(()=>{const q=normalizar(pesquisa.trim());if(!q)return produtos;return produtos.filter(p=>normalizar(`${p.nome} ${p.sku||''} ${categorias.find(c=>c.id===p.categoria_id)?.nome||''} ${fornecedores.find(f=>f.id===p.fornecedor_id)?.nome||''}`).includes(q))},[produtos,pesquisa,categorias,fornecedores])
