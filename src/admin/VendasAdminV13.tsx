...
function VendasAdminV13() {
  ...
  async function carregar() {
    ...
    await filaVenda;
    const [c, p2, vend] = await Promise.all([supabase.from("clientes").select("id,nome").eq("ativo", true).order("nome"), supabase.from("produtos").select("id,nome,sku,preco,preco_promocional,promocao,produto_variantes(id,tamanho,estoque,ativo)").eq("ativo", true).order("nome"), supabase.from("vendedores_v17_30").select("id,nome,comissao_percentual").eq("ativo", true).order("nome")]);
    if (c.error || p2.error || vend.error) setErro(c.error?.message || p2.error?.message || vend.error?.message || "Erro ao carregar.");
    else {
      const ps = p2.data || [];
      setClientes(c.data || []);
      setVendedores(vend.data || []);
      setProdutos(ps);
      const d = await supabase.rpc("obter_rascunho_venda_v17_30");
      if (d.error) setErro("Aplique o SQL V17.28 antes de usar: " + d.error.message);
      else {
        const r = d.data;
        setRascunhoId(r.id);
  ...
  useEffect(() => {
    if (!pronto.current || !rascunhoId) return;
    setNuvem("Salvando...");
    const a = args();
    filaVenda = filaVenda.then(async () => {
      try {
        await persistir(a);
        setNuvem("Salvo na nuvem");
      } catch (e) {
        setNuvem("Falha ao salvar");
        setErro("Falha ao salvar o rascunho: " + e.message);
      }
    });
  }, [rascunhoId, cliente, vendedor, data, forma, desconto, entrega, obs, parcelas, vencimento, valoresParcelas, itens]);
  const p = useMemo(() => produtos.find((x) => x.id === produtoId), [produtos, produtoId]), vars = (p?.produto_variantes || []).filter((v) => v.ativo && v.estoque > 0), produtosFiltrados = useMemo(() => {
    const q = buscaProduto.trim().toLocaleLowerCase("pt-BR");
    return produtos.filter((x) => x.produto_variantes.some((v) => v.ativo && v.estoque > 0) && (!q || x.nome.toLocaleLowerCase("pt-BR").includes(q) || (x.sku || "").toLocaleLowerCase("pt-BR").includes(q)));
  }, [produtos, buscaProduto]);
  function selProduto(id) {
    setProdutoId(id);
    setVarianteId("");
    const x = produtos.find((p2) => p2.id === id);
    setPreco(Number(x?.promocao && x.preco_promocional != null ? x.preco_promocional : x?.preco) || 0);
  }
  function adicionar() {
    const p2 = produtos.find((x) => x.id === produtoId), v = p2?.produto_variantes.find((x) => x.id === varianteId);
    if (!p2 || !v) {
      setErro("Selecione produto e tamanho.");
      return;
    }
    const ja = itens.find((i) => i.variante_id === v.id)?.quantidade || 0;
    if (qtd < 1 || qtd + ja > v.estoque) {
      setErro(`Estoque dispon\xEDvel: ${v.estoque}.`);
      return;
    }
    setItens((a) => {
      const i = a.find((x) => x.variante_id === v.id);
      return i ? a.map((x) => x.variante_id === v.id ? { ...x, quantidade: x.quantidade + qtd, preco_unitario: preco } : x) : [...a, { variante_id: v.id, produto: p2.nome, tamanho: v.tamanho, quantidade: qtd, preco_unitario: preco }];
    });
    setVarianteId("");
    setQtd(1);
    setErro("");
  }
  const subtotal = itens.reduce((a, i) => a + i.quantidade * i.preco_unitario, 0), total = Math.max(0, subtotal - desconto + entrega);
  useEffect(() => {
    if (forma !== "Credi\xE1rio") return;
    const centavos = Math.round(total * 100), base = Math.floor(centavos / Math.max(1, parcelas)), resto = centavos - base * Math.max(1, parcelas);
    setValoresParcelas((atuais) => atuais.length === parcelas && Math.round(atuais.reduce((a, v) => a + Number(v || 0), 0) * 100) === centavos ? atuais : Array.from({ length: Math.max(1, parcelas) }, (_, i) => (base + (i === parcelas - 1 ? resto : 0)) / 100));
  }, [forma, parcelas, total]);
  async function salvar(e) {
    ...
    try {
      ...
      setErro(e2 instanceof Error ? e2.message : "Erro ao concluir venda.");
    } finally {
      setSaving(false);
    }
  }
  async function cancelarRascunho() {
    if (!confirm("Cancelar esta venda? Todos os dados do rascunho ser\xE3o descartados.")) return;
    setSaving(true);
    try {
      await filaVenda;
      const r = await supabase.rpc("cancelar_rascunho_venda_v17_28", { p_rascunho_id: rascunhoId });
      if (r.error) throw r.error;
      setOk("Rascunho da venda cancelado.");
      await carregar();
    } catch (e) {
      setErro(e.message);
    } finally {
